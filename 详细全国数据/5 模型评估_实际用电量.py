...
import joblib  # 导入joblib模块来加载归一化模型
...
for i, name in enumerate(target_names):
    ...
    plt.plot(test_targets[:, i], label='Actual Electricity Consumption (' + name + ')')
    plt.plot(predictions_inversed[:, i], label='Predicted Electricity Consumption (' + name + ')')
    plt.title('Prediction vs Actual for Electricity Consumption: ' + name)
